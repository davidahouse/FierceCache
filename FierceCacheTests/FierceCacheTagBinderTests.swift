...
class FierceCacheTagBinderTests: XCTestCase {
    ...
    func testOnInsert() {
        ...
        let binder = cache.bindTag("strings")
        binder.onInsert = { (objects:[(String,Any)],notification:FierceCacheNotificationType) in
            
            if let result = objects[0].1 as? String {
        ...
        cache.set("strings_1", object:payload, tags:["strings"])
    ...
    func testOnUpdate() {
        ...
        let binder = cache.bindTag("strings")
        binder.onUpdate = { (objects:[(String,Any)],notification:FierceCacheNotificationType) in
        ...
        cache.set("strings_1", object:payloadUpdated, tags:["strings"])
    ...
    func testOnDelete() {
        ...
        let binder = cache.bindTag("strings")
        binder.onDelete = { (objects:[(String,Any)],notification:FierceCacheNotificationType) in
            
            if ( objects.count == 0 ) {
                expectation.fulfill()
            }
            else {
                print("notification had \(objects.count) objects in it")
            }
        }
